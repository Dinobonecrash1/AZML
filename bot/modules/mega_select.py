#!/usr/bin/env python3
"""
Mega Folder File Selection — Web selection UI.
"""
...
def mega_selection_buttons(uid: int):
    gid = str(uid)
    pincode = "".join([n for n in gid if n.isdigit()][:4])
    buttons = ButtonMaker()
    BASE_URL = config_dict["BASE_URL"]
    buttons.ubutton("Select Files", f"{BASE_URL}/app/files/{gid}")
    buttons.ibutton("Pincode", f"megasel pin {gid} {pincode}")
    buttons.ibutton("Cancel", f"megasel cancel {gid}")
    buttons.ibutton("Done Selecting", f"megasel done {gid}")
    return buttons.build_menu(2)

@new_task
async def mega_select_cb(client, query):
    """Handle all megasel callback queries."""
    from bot.helper.mirror_utils.download_utils.mega_download import mega_select_cache
    ...
    cache = mega_select_cache.get(uid)
    if cache is None:
    ...
    if action == 'pin':
        ...
        selected_ids = MegaSelectionStore().read_selection(str(uid))
        ...
        LOGGER.info(
            f"[MEGA SELECT] uid={uid} user={user_id} "
            f"selected {len(selected)}/{len(cache['files'])} files "
            f"({get_readable_file_size(total_sel_size)})"
        )
        await deleteMessage(query.message)
        MegaSelectionStore().delete_data(str(uid))
        cache['event'].set()

    elif action == 'cancel':
        await query.answer("❌ Download cancelled!")
        LOGGER.info(f"[MEGA SELECT] uid={uid} user={user_id} cancelled selection")
        cache['is_cancelled'] = True
        await deleteMessage(query.message)
        MegaSelectionStore().delete_data(str(uid))
        cache['event'].set()

bot.add_handler(CallbackQueryHandler(mega_select_cb, filters=regex(r'^megasel')))
